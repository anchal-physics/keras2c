...
/**
 * 1D (temporal) Insert Zeros.
 * Inserts zeros between each row of the input tensor.
 *
 * :param output: tensor to store output data with inserted zeros.
 * :param input: tensor to insert
 * :param zeros_ins: array[2] of how many rows to insert in between each row. Order is {before dim 1, after dim 1}.
 */
void k2c_insert_zeros1D(k2c_tensor *output, const k2c_tensor *input, const size_t zeros_ins)
{

    const size_t in_height = input->shape[0];
    const size_t in_width = input->shape[1];
    const size_t jump = zeros_ins + 1;
    ...
    for (size_t i = 0; i < in_height; ++i)
    {
        for (size_t k = 0; k < in_width; ++k)
        {
            output->array[jump * i * in_width + k] = input->array[i * in_width + k];
        }
    }
}

/**
 * 2D (spatial) Insert Zeros.
 * Inserts zeros between each row and column of the input tensor.
 *
 * :param output: tensor to store output data with inserted zeros.
 * :param input: tensor to insert
 * :param zeros_ins: array[4] of how many rows/cols to insert in between each row/col. Order is {before dim 1, after dim 1, before dim 2, after dim 2}.
 */
void k2c_insert_zeros2D(k2c_tensor *output, const k2c_tensor *input, const size_t *zeros_ins)
{

    const size_t in_height = input->shape[0];
    const size_t in_width = input->shape[1];
    const size_t channels = input->shape[2];
    const size_t jump1 = zeros_ins[0] + 1;
    const size_t jump2 = zeros_ins[1] + 1;
    ...
    for (size_t i = 0; i < in_height; ++i)
    {
        ...
        {
            const size_t insub[K2C_MAX_NDIM] = {i, j, 0};
            const size_t outsub[K2C_MAX_NDIM] = {i * jump1, j * jump2, 0};
            memcpy(&output->array[k2c_sub2idx(outsub, output->shape, output->ndim)],
                   &input->array[k2c_sub2idx(insub, input->shape, input->ndim)],
                   channels * sizeof(input->array[0]));
        }
...
/**
 * 3D (spatial or spatio-temporal) Insert Zeros.
 * Inserts zeros between each row, column and depth of the input tensor.
 *
 * :param output: tensor to store output data with inserted zeros.
 * :param input: tensor to insert
 * :param zeros_ins: array[6] of how many rows/cols to insert in between each row/col. Order is {before dim 1, after dim 1, before dim 2, after dim 2, before dim 3, after dim 3}.
 */
void k2c_insert_zeros3D(k2c_tensor *output, const k2c_tensor *input, const size_t *zeros_ins)
{
    ...
    const size_t jump1 = zeros_ins[0] + 1;
    const size_t jump2 = zeros_ins[1] + 1;
    const size_t jump3 = zeros_ins[2] + 1;
    ...
    for (size_t i = 0; i < dim1; ++i)
    {
        for (size_t j = 0; j < dim2; ++j)
        {
            for (size_t k = 0; k < dim3; ++k)
            {
                const size_t insub[K2C_MAX_NDIM] = {i, j, k, 0};
                const size_t outsub[K2C_MAX_NDIM] = {i * jump1, j * jump2, k * jump3, 0};
                memcpy(&output->array[k2c_sub2idx(outsub, output->shape, output->ndim)],
                       &input->array[k2c_sub2idx(insub, input->shape, input->ndim)],
                       channels * sizeof(input->array[0]));
            }
